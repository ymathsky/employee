<?php
// FILENAME: employee/api/add_announcement.php
session_start();
...
try {
    $sql = "INSERT INTO announcements (title, content, created_by_id) VALUES (?, ?, ?)";
    $stmt = $pdo->prepare($sql);
    $stmt->execute([$data['title'], $data['content'], $admin_id]);

    log_action($pdo, $admin_id, 'ANNOUNCEMENT_CREATED', "New announcement created: {$data['title']}");
