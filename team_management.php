<?php
// FILENAME: employee/team_management.php
...
try {
    ...
    error_log("Error fetching manager department for Team Management: " . $e->getMessage());
}

// Function to fetch employees based on role/department
function getTeamEmployees($pdo, $department, $is_admin) {
    try {
        // --- MODIFIED: Added e.profile_picture_url to SELECT ---
        $sql = "SELECT e.*, u.role, e.profile_picture_url
                 FROM employees e
                 LEFT JOIN users u ON e.employee_id = u.employee_id";
        $params = [];
        if (!$is_admin && !empty($department)) {
            // Use LIKE for multi-department support (e.g., "HR, IT" matches "HR")
            $sql .= " WHERE e.department LIKE ?"; 
            $params[] = "%" . $department . "%";
        }
        // Do NOT exclude the manager here, as Admins can see everyone, and Managers might need their own profile if they are listed.
        ...
        error_log("Error fetching team employees: " . $e->getMessage());
        return [];
    }
}
...
<div class="bg-white p-8 rounded-xl shadow-xl">
    ...
    <?php if (!$is_admin && empty($manager_department)): ?>
        <div class="p-4 bg-red-100 text-red-700 rounded-lg mb-4">
            Warning: You are a Manager but are not currently assigned a department in the employee records. Showing no employees.
        </div>
    <?php endif; ?>
                ...
                <?php foreach ($employees as $employee): ?>
                    <tr>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                            <!-- MODIFIED: Added profile picture thumbnail -->
                            <div class="flex items-center">
                                <?php
                                $team_pic_src = !empty($employee['profile_picture_url'])
                                    ? '../' . htmlspecialchars($employee['profile_picture_url'])
                                    : 'https://placehold.co/40x40/667eea/ffffff?text=' . strtoupper(substr($employee['first_name'] ?? 'E', 0, 1));
                                ?>
                                <img src="<?php echo $team_pic_src; ?>" alt="Profile" class="h-10 w-10 rounded-full object-cover mr-3 flex-shrink-0">
                                <div>
                                    <?php echo htmlspecialchars($employee['first_name'] . ' ' . $employee['last_name']); ?>
                                    <span class="text-xs text-gray-500 block">ID: <?php echo $employee['employee_id']; ?></span>
                                </div>
                            </div>
                            <!-- END MODIFIED -->
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            ...
                            <!-- Only show Set Rate button to Admins/Managers, and managers are restricted to their team by the API -->
                            <?php if (!$is_admin && !empty($manager_department)): ?>
